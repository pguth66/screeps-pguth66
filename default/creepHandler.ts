...
interface CreepG extends Creep {

}
Creep.prototype.creepLog = function (text: string) {
...
Creep.prototype.respawn = function () {
    var body = [];
    var newCreepMemory: CreepMemory = { role: this.memory.role, respawn: true };
    ...
    try {
        switch (this.memory.role) {
            ...
            case 'archer':
                ...
                break;
            case 'contracthauler':
                const memProps = ['pullTarget', 'dropTarget', 'job', 'upTo', 'resource', 'total', 'taskID', 'loadingTerminal', 'unloadingTerminal', 'processed'];
    ...
    handleCreep: function (creep) {
        ...
        if (!creep.spawning) {
            ...
            if (typeof creep.memory.needsBoost !== 'undefined' && creep.memory.needsBoost.length > 0) {
                const boostTypes = ['armor', 'attack', 'move', 'heal'];
            ...
            if (creep.memory.targetRoom && (creep.memory.targetRoom != creep.room.name)) {
                ...
                for (let i = 0; i < creepRoles.length; i++) {
