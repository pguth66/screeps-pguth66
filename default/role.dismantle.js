...
var roleDismantle = {
    run: function (creep) {
        ...
        var dismantleFlags = creep.room.find(FIND_FLAGS, { filter: { color: COLOR_RED } });
        if (dismantleFlags.length > 0) {
            var dismantleFlag = creep.pos.findClosestByPath(dismantleFlags);
