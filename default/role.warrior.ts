...
var roleWarrior:any = {
  ...
  /** @param {Creep} creep **/
    run: function(creep) {
        ...
        const hostileTowers = _.remove(hostiles, {structureType: STRUCTURE_TOWER});
        const hostileSpawns = _.remove(hostiles, {structureType: STRUCTURE_SPAWN});
        const powerBank = creep.room.find(FIND_STRUCTURES, {filter: {structureType: STRUCTURE_POWER_BANK}});

        const typesToRemove = [ STRUCTURE_RAMPART, STRUCTURE_CONTROLLER, STRUCTURE_STORAGE, STRUCTURE_TERMINAL ];
...
//        _.remove(hostiles, {structureType: STRUCTURE_CONTROLLER}); 
        ...
        if(closestHostile) {
            ...
            if((creep.room != Game.rooms[Memory.roomToAttack]) && (creep.room.name != creep.memory.targetRoom)) {
                ...
                const attackFlags = creep.room.find(FIND_FLAGS, {filter: {color: COLOR_BROWN}});
                if(attackFlags.length > 0) {
                    const closestFlag = creep.pos.findClosestByPath(attackFlags);
                    //creep.creepLog(' closestFlag is ' + closestFlag);
                    creep.say('Flag!');
                    if(closestFlag) {
                        const targetArr=closestFlag.pos.findInRange(FIND_STRUCTURES,0);
