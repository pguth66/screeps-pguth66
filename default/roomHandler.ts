...
Room.prototype.getMinsFromNearestRoom = function (mineral) {
    const roomsWithMin = _.filter(Game.rooms, (r) => { if (r.controller && r.controller.my && r.terminal ) { return r.terminal.store[mineral] > 10000 } });
    var destRoomMatrix = [];
    roomsWithMin.forEach(function (room, i) {
        if (room.terminal) {
            destRoomMatrix[i] = { 'room': room, 'dist': Game.map.getRoomLinearDistance(this.name, room.name, true) };
...
Room.prototype.addToCreepBuildQueue = function (creepType, memoryObject: CreepMemory, name?: String) {
    //console.log('adding creep to build queue: '+ creepType);
...
Room.prototype.getSoldierBody = function (parts) {
    ...
    for (let i = 0; i < parts.attack; i++) {
        newbody.push(ATTACK);
    }
    for (let i = 0; i < parts.heal; i++) {
        newbody.push(HEAL);
    }
    for (let i = 0; i < parts.move; i++) {
        newbody.push(MOVE);
    }
    for (let i = 0; i < parts.rangedAttack; i++ ) {
...
Room.prototype.getCreepBody = function (role: string, targetRoom: string ) {
    ...
    if (targetRoom && (role == 'harvester' || role == 'upgrader')) {
        var room: Room = Game.rooms[targetRoom];
    }
    else {
        var room: Room = this;
    }
...
function nearestRoom(rooms: Room[], targetRoom: Room, method: String) {
    ...
    rooms.forEach(function (room, i) {
        switch (method) {
            case 'linear':
                ...
                break;
            case 'path':
                let dist = Game.map.findRoute(this.name, room.name);
                switch (dist) {
                    case -2: 
...
Room.prototype.findRoomToSendAttackSquad = function () {
    var sourceRooms = _.filter(Game.rooms, (r) => { return r.controller && r.controller.level == 8 && r.storage.store[RESOURCE_ENERGY] > 200000 && r.energyAvailable > 10000 })
